...
A React frontend served by the FastAPI backend runs the full verification flow end-to-end.

Selecting a user and clicking **Verify** runs one *genuine* attempt (that user's own motion sample) plus one *impostor* attempt for every other enrolled user — each scored against the **claimed user's** fine-tuned model. That is precisely the attack the system exists to stop: someone else's motion presented under your identity. The result page reports the genuine decision, every impostor decision, and the measured FAR across the run.

```
HomePage                          VerificationPage
─────────                         ────────────────
select claimed identity    ──▶    genuine attempt  (score dial vs threshold)
click Verify                      summary          (tested / rejected / accepted / FAR)
                                  impostor grid    (one card per other user)
```
...
> **Note:** the UI screenshots (`01`–`04`) predate the verification-flow rewrite and show the retired two-page demo (`DemoPage` → `ResultPage`). They need retaking against the current `HomePage` → `VerificationPage` flow.
...
Open http://localhost:8000 — select a user, click **Verify**.
...
├── frontend/
│   └── src/
│       ├── pages/                # HomePage, VerificationPage
│       └── components/           # ScoreDial
├── evaluation/
...
├── screenshots/
│   ├── 01_homepage.png           # ⚠ pre-rewrite (retired demo flow)
│   ├── 02_uv_score_dial.png      # ⚠ pre-rewrite
│   ├── 03_accept_result.png      # ⚠ pre-rewrite
│   ├── 04_reject_result.png      # ⚠ pre-rewrite
...
| `POST /predict/demo/{user_id}` | Full verification — genuine attempt + one impostor per other user, with FAR summary |
| `POST /predict/mpi` | MPI stage alone on a 3-second sensor window |

`POST /predict/demo/{user_id}` returns:
...
```jsonc
{
  ...
  "summary":  { "total_impostors": 10, "correctly_rejected": 10,
                "incorrectly_accepted": 0, "far": 0.0, "far_display": "0.00% · 1/∞" }
}
```

This endpoint runs the **UV stage only** — every attempt is scored against the claimed user's fine-tuned model. The MPI stage is exercised separately via `POST /predict/mpi`.
